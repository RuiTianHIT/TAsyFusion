...
def inv_depth_to_depth(prediction, reg_factor=3.70378):
    # convert to normalize depth (target is coming in log inverse depth)
    prediction = np.exp(
        reg_factor * (prediction - np.ones((prediction.shape[0], prediction.shape[1]), dtype=np.float32)))
...
def display_high_contrast_colormap(idx, target, prediction, prefix="", colormap='terrain', debug=False,
                                   folder_name=None):
    if folder_name is not None or debug:
        percent = 1.0
        fig, ax = plt.subplots(ncols=1, nrows=2)
...
def display_high_contrast_color_logmap(idx, data, prefix="", name="data", colormap='tab20c', debug=False,
                                       folder_name=None):
    if debug and folder_name is not None:
        percent = 1.0
        fig, ax = plt.subplots(ncols=1, nrows=1)
...
if __name__ == "__main__":
    ...
    for idx in tqdm.tqdm(range(num_it)):
        ...
        debug = flags.debug and idx == flags.idx
        # print(debug)
        metrics = add_to_metrics(idx, metrics, target_depth, predicted_depth, valid_mask, event_frame=None, prefix="_",
                                 debug=debug, output_folder=flags.output_folder)
