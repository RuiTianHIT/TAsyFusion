...
#
#     test_dataset = SequenceMVSEC(base_folder="/mnt/wwn-0x50014eef01564214-part1/DENSE/test_sequence_00_town10/",
#                                  sequence_length=L,
#                                  transform=None,
#                                  step_size=L,
...
#             rgb_image = item[0]['rgb'].unsqueeze(dim=0)
#             text = item[0]['text'].unsqueeze(dim=0)
#
#             target = item[0]['depth'].cpu().numpy()
#             # print(target.shape)
#
#             events = events.float().to(gpu)
#             rgb_image = rgb_image.float().to(gpu)
#
#             pred_dict = model(events, rgb_image, text)
...
#                         default="/mnt/wwn-0x50014eef01564214-part1/e2depth_checkpoints_2sota/EReFormer_dense_epoch200_000032/model_best.pth.tar")
...
#
#     path_to_model = '/mnt/wwn-0x50014eef01564214-part1/e2depth_checkpoints_2sota/EReFormer_dense_epoch200_000032/model_best.pth.tar'
#
...
def main(config, initial_checkpoint):
    ...
    with torch.no_grad():
        ...
        while idx < N:
            ...
            events = events.float().to(gpu)
            text = item[0]['text'].unsqueeze(dim=0)
            image_color = item[0]['event'].unsqueeze(dim=0)
            image_color = image_color.float().to(gpu)

            # fake_events = torch.randn((1, 3, 224, 224)).float().to(gpu)
            # fake_image_color = torch.randn((1, 3, 224, 224)).float().to(gpu)
            # fake_text = torch.randn((1, 1, 768)).float().to(gpu)
            #
            # macs, params = profile(model, inputs=(fake_events, fake_image_color, fake_text), verbose=False)
            # macs, params = clever_format([macs, params], "%.3f")
            # print(f"模型的总计算量 (MACs): {macs}")
            # print(f"模型的总参数量 (Params): {params}")

            pred_dict = model(events, image_color, text)
            pred_depth = pred_dict['pred_depth']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str,
                        help='path to config. If not specified, config from model folder is taken',
                        default="./configs/EReFormer_dense_epoch200_000032.json")
    parser.add_argument('--data_folder', type=str,
                        help='path to folder of data to be tested',
                        default=None)
    ...
    path_to_model = '/home/server/tianrui/EReFormer_new/e2depth_checkpoints/EReFormer_dense_epoch200_000032/model_best_12.449961914102202.pth.tar'
